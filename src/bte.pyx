...
class Lineage:
    """
    Class container for lineage annotations. 
    """
    def __init__(self, t: MATree, nid: str):
        self.parent = t.get_node(nid).most_recent_annotations()
        self.path = [n.mutations for n in t.rsearch(nid,True)[::-1]]
        self.samples = t.get_leaves_ids(nid)
        self.aliases = []
        self.nid = nid

    def __repr__(self):
        fstr = ""
        fstr += "parent lineages: " + ",".join(self.parent) + "\n"
        fstr += "mutation path: " + " > ".join([",".join(ms) for ms in self.mutations]) + "\n"
        fstr += "sample count: " + str(len(self.samples)) + "\n"
        fstr += "aliases: " + ",".join(self.aliases) + "\n"
        fstr += "node id: " + self.nid + "\n"
        return fstr
...
cdef class MATNode:
    ...
    cdef from_node(self, bte.Node* n):
        ...
        """
        if n == cython.NULL:
            raise ValueError("Target node is null- check that your node name exists and that you're not taking the parent of the root node.")
        self.n = n
        return self
    ...
    def __init__(self, pb_file: Optional[str] = None, uncondense: bool = True, nwk_file: Optional[str] = None, nwk_string: Optional[str] = None, vcf_file: Optional[str] = None, json_file: Optional[str] = None) -> None:
        ...
        if pb_file != None:
            ...
            if vcf_file != None:
                ...
                print("WARNING: nwk_string is for tree-only loading and does not use vcf input. Consider usingi nwk_file.",file=sys.stderr)
